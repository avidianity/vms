    ...
    if (error) {
        if (error.response) {
            ...
            if (response.data.errors && isArray<string>(response.data.errors)) {
                return Object.values<string[]>(response.data.errors).map(
                    (errors) => errors.map((error) => toastr.error(error)),
                );
            }
...
export function errorToStrings(error: any) {
    if (error) {
        if (error.response) {
            ...
            if (response.data.message) {
                if (isArray(response.data.message)) {
                    return (response.data.message as string[]).map((message) =>
                        sentencify(message),
                    );
                } else if (isString(response.data.message)) {
            ...
            if (isString(error.message)) {
                ...
                return (error.message as string[]).map((message) =>
                    sentencify(message),
                );
            }
        }
